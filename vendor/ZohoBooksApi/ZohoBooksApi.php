...
class ZohoBooksApi {
  ...
  protected function prepareMethods() {
    ...
    foreach ($this->smartObjects as $object) {
      ...
      foreach ($this->smartActions as $action => $httpMethod) {
        ...
        // append method
        $this->methods[$object . $action] = array(
          'url' => $url,
          'method' => $httpMethod,
        );
      }
  ...
  public function makeApiRequest(
    ...
    if (!in_array($method, array('GET', 'POST', 'PUT', 'DELETE'))) {
      throw new ZohoBooksApiException(
        'incorrect method requested - "' . $method . '"', 1
      );
    }
    ...
    if ($this->lastRequest['dataRaw'] === FALSE) {
      throw new ZohoBooksApiException(
        'request timed out [' . $method . ' ' . $fullURL . ']', 11
      );
    }
    ...
    if (substr($this->lastRequest['httpCode'], 0, 1) != '2') {
      throw new ZohoBooksApiHttpException(
        'HTTP code "' . $this->lastRequest['httpCode'] . '" - waiting for 2XX',
        $this->lastRequest['httpCode']
      );
    }
    ...
    // parse json
    $this->lastRequest['data'] = @json_decode(
      $this->lastRequest['dataRaw'],
      TRUE
    );
  ...
  public function __call($method, $args) {
    ...
    if (!array_key_exists($method, $this->methods)) {
      throw new ZohoBooksApiException(
        'requested method "' . $method . '" does not exists', 2
      );
    }
    ...
    if ($paramsCount > 0) {
      // replace params
      $methodInfo['url'] = vsprintf(
        $methodInfo['url'],
        array_slice($args, 0, $paramsCount)
      );
    ...
    if (!is_array($query)) {
      throw new ZohoBooksApiException(
        'query data should be array, ' . gettype($query) . ' received', 4
      );
    }

    // call this method and get response
    $R = $this->makeApiRequest(
      $methodInfo['url'],
      $methodInfo['method'],
      $query,
      isset($methodInfo['raw']) && $methodInfo['raw']
    );
